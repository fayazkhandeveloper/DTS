...
function FnLoadRecordData() {
    var param = [];
    var proc = 'Restaurant_Record_ID';
    Selector(param, proc).then(function (result) {
        //$res = JSON.parse(result);
        $res = JSON.parse(result);

        if ($res["dt2"][0].RowCount == null || $res["dt2"][0].RowCount == "" || $res["dt2"][0].RowCount == undefined) { RecordId = $res["dt1"][0].RowCount; }
        ...
        if (files.length === 0 || files.length === undefined || files.length === null) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    { "name": "Info_ID", "Value": BusinessId },
                    { "name": "FileType", "Value": "Images" },
                    { "name": "UpdatedFile", "Value": "RestaurantImages/" + $res[0].ImageName.replace(/ /g, "_") }];

                    var proc = 'upload_restaurant_images_insert';
                    NewAction(param, proc).then(function (result) {
                        ...
                        if (result == 'Operation Successfully Performed') {
                            ...
                            rowCount++;
                            window.location.href = "Restaurant_Registration";
                        }
...
function FnLoadMasterData() {
    ...
    var proc = 'restaurant_master_load_data';
    Selector(param, proc).then(function (result) {
        ...
        if ($res["dt3"][0].RowCount == null || $res["dt3"][0].RowCount == "" || $res["dt3"][0].RowCount == undefined) { BusinessId = $res["dt4"][0].RowCount; }
...
function FnInsertRestaurantInfo() {
    LoaderFadeIn();
    var param = [{ "name": "Info_ID", "Value": BusinessId },
    { "name": "Name", "Value": $("#h_name").val() },
    { "name": "Province_ID", "Value": $("#DDLProvinces").val() },
    { "name": "District_ID", "Value": $("#DDLDistrict").val() },
    { "name": "Air_Condition", "Value": $("#DDlRestaurantTypee").val() },
    { "name": "No_of_Seats", "Value": $("#DDlRestaurantType").val() },
    { "name": "Email_ID", "Value": $("#email").val() },
    ...
    var proc = 'Restaurant_Info_Insert';
...
function FnInsertFacilities() {
    ...
    var proc = 'Restaurant_Facilities_Insert';
    Action(param, proc).then(function (result) {
